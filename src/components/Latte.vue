...
export default {
  ...
  methods: {
    saveTitle (index) {
      this.$firebase.database().ref().child('menu').child('latte').child(index).update({ title: this.menuTitle })
      this.selectedItemTitle = this.menuTitle
    },
    saveDescription (index) {
      this.$firebase.database().ref().child('menu').child('latte').child(index).update({ description: this.menuDescription })
