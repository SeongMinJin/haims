...
export default {
  ...
  methods: {
    saveTitle () {
      this.$firebase.database().ref().child('menu').child('latte').child(this.selectedItemIndex).update({ title: this.selectedItemTitle })
    },
    saveDescription () {
      this.$firebase.database().ref().child('menu').child('latte').child(this.selectedItemIndex).update({ description: this.selectedItemDescription })
    },
    savePrice () {
      this.menuPrice = this.selectedItemPrice
      this.$firebase.database().ref().child('menu').child('latte').child(this.selectedItemIndex).update({ price: this.selectedItemPrice })
    },
    saveImage () {
      this.$firebase.database().ref().child('menu').child('latte').child(this.selectedItemIndex).update({ picture: this.menuImageURL })
    },
    updateMenu () {
      if (!this.latte) {
        this.emptyLatte.push({
          title: this.menuTitle,
          description: this.menuDescription,
          price: this.price,
          picture: this.menuImageURL
        })
        this.$firebase.database().ref().child('menu').update({ latte: this.emptyLatte })
      } else {
        this.latte.push({
          title: this.menuTitle,
          description: this.menuDescription,
        ...
        this.$firebase.database().ref().child('menu').update({ latte: this.latte })
      }
      this.menuTitle = ''
    ...
    async updateMenuImage () {
      var sn = await this.$firebase.storage().ref().child('latte').child(this.menuTitle).put(this.menuImage)
    ...
    openEditDialog (index) {
      ...
      this.selectedItemTitle = this.latte[this.selectedItemIndex].title
      this.selectedItemDescription = this.latte[this.selectedItemIndex].description
      this.selectedItemPrice = this.latte[this.selectedItemIndex].price
    },
    closeDialog () {
      this.edit = false
      this.del = false
    ...
    deleteMenu (index) {
      var title = this.latte[index].title
      this.latte.splice(index, 1)
      this.del = false
      this.$firebase.database().ref().child('menu').update({ latte: this.latte })
      this.$firebase.storage().ref().child('latte').child(title).delete()
    }
  }
}
</script>
