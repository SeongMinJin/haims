...
export default {
  ...
  methods: {
    saveTitle () {
      this.$firebase.database().ref().child('menu').child('coffee').child(this.selectedItemIndex).update({ title: this.selectedItemTitle })
    },
    saveDescription () {
      this.$firebase.database().ref().child('menu').child('coffee').child(this.selectedItemIndex).update({ description: this.selectedItemDescription })
    },
    savePrice () {
      this.menuPrice = this.selectedItemPrice
      this.$firebase.database().ref().child('menu').child('coffee').child(this.selectedItemIndex).update({ price: this.selectedItemPrice })
    },
    saveImage () {
      this.$firebase.database().ref().child('menu').child('coffee').child(this.selectedItemIndex).update({ picture: this.menuImageURL })
    },
    updateMenu () {
      if (!this.coffee) {
    ...
    async updateMenuImage () {
      var sn = await this.$firebase.storage().ref().child('coffee').child(this.menuTitle).put(this.menuImage)
    ...
    openEditDialog (index) {
      ...
      this.selectedItemPrice = this.coffee[this.selectedItemIndex].price
    },
    closeDialog () {
      this.edit = false
      this.del = false
    ...
    deleteMenu (index) {
      var title = this.coffee[index].title
      this.coffee.splice(index, 1)
      this.del = false
      this.$firebase.database().ref().child('menu').update({ coffee: this.coffee })
      this.$firebase.storage().ref().child('coffee').child(title).delete()
    }
  }
}
</script>
