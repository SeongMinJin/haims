...
export default {
  ...
  methods: {
    saveTitle () {
      this.$firebase.database().ref().child('menu').child('ade').child(this.selectedItemIndex).update({ title: this.selectedItemTitle })
    },
    saveDescription () {
      this.$firebase.database().ref().child('menu').child('ade').child(this.selectedItemIndex).update({ description: this.selectedItemDescription })
    },
    savePrice () {
      this.menuPrice = this.selectedItemPrice
      this.$firebase.database().ref().child('menu').child('ade').child(this.selectedItemIndex).update({ price: this.selectedItemPrice })
    },
    saveImage () {
      this.$firebase.database().ref().child('menu').child('ade').child(this.selectedItemIndex).update({ picture: this.menuImageURL })
    },
    updateMenu () {
      if (!this.ade) {
        this.emptyAde.push({
        ...
        this.$firebase.database().ref().child('menu').update({ ade: this.emptyAde })
      } else {
        this.ade.push({
          title: this.menuTitle,
          description: this.menuDescription,
        ...
        this.$firebase.database().ref().child('menu').update({ ade: this.ade })
      }
      this.menuTitle = ''
    ...
    async updateMenuImage () {
      var sn = await this.$firebase.storage().ref().child('ade').child(this.menuTitle).put(this.menuImage)
    ...
    openEditDialog (index) {
      ...
      this.selectedItemTitle = this.ade[this.selectedItemIndex].title
      this.selectedItemDescription = this.ade[this.selectedItemIndex].description
      this.selectedItemPrice = this.ade[this.selectedItemIndex].price
    },
    closeDialog () {
      this.edit = false
      this.del = false
    ...
    deleteMenu (index) {
      var title = this.ade[index].title
      this.ade.splice(index, 1)
      this.del = false
      this.$firebase.database().ref().child('menu').update({ ade: this.ade })
      this.$firebase.storage().ref().child('ade').child(title).delete()
    }
  }
}
</script>
